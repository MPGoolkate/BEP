...
def liftBlossom(G, blossom):
    ...
    for neighbor in G.get_neighbors(blossom_repr): # max O(n) times so O(n) in total
        if not neighbor.blossomRoot == blossom_repr.blossomRoot and G.matching[neighbor.name] != -1: # O(1) 
            stem = neighbor # O(1) 
            break
...
def updateNeighbors(G, blossom):
    ...
    G = updateV(G, blossom_repr, blossom_repr, checked) # O(m)

    for neighbor in blossom_repr.blossomEdges:
        obj = G.get_neighbors(neighbor, False).get_first() # O(1)
        while obj != None: 
            if obj.vertex.blossomRoot != blossom_repr.blossomRoot:
                neighbor.blossomEdges.add(obj) # O(1
            obj = obj.next
        neighbor.blossomEdges.add(blossom_repr) # O(1)
    
    return G
    
def updateV(G, blossom_repr, v, checked):
    #blossom_repr.blossomEdges = G.get_neighbors(blossom_repr).copy() # O(n)
    # total of O(m) time
    if not checked[v.name]: # if not check before
        ...
        if v.blossomRoot == blossom_repr.blossomRoot: 
            ...
            for neighbor in G.edges[v.name]: # O(n)
                G = updateV(G, blossom_repr, neighbor, checked) # O(m)
        else:
            # v is not in the blossom so add it to the list
            blossom_repr.blossomEdges.add(v) # O(1)
    return G
