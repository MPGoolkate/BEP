...
def invertPath(M, path):
    for i in range(len(path)): # we loop over the vertices in the path of lenght max n
        if i % 2 == 0 and i < len(path) - 1: # O(1) check if even using bitwise operator and check length
...
# Goal: lift a blossom
# Running time: O(n^2) where n is the number of vertices in the graph
def liftBlossom(G, M, blossom, path):
    # first, we find the path including the path within the blossom
    base = blossom.vertices[-1] # O(1)

    # find the stem of the blossom
    if path is not None: #O(1)
        if base not in path: #O(n)
            # blossom is not in the path
            return path #O(1)
        else:
            # blossom is in the path
            stemL = path[:path.index(base)] or [] #O(n)
            stemR = path[path.index(base)+1:] or [] #O(n)
    else:
        return None

    # find the two vertices connected to the blossom
    connected_vert_r = None #O(1)
    connected_vert_l = None #O(1)
    for vertex in blossom.vertices: #O(n) times (length of the blossom) so whole loop is O(n^2)
        if len(stemL) > 0 and stemL[-1].name in blossom.edges[vertex.name]: #O(n)
            connected_vert_l = vertex #O(1)
        if len(stemR) > 0 and stemR[0].name in blossom.edges[vertex.name]: #O(n)
            connected_vert_r = vertex #O(1)

    # find the correct path within the blossom -> O(n)
    blossom_path = [] #O(1)
    if connected_vert_l is not None and connected_vert_r is not None: #O(1)
        if stemL[-1] in M.vertices: #O(n)
            # we need to take matched edge first
            if base in blossom.matching_vertices: #O(n)
                blossom_path = stemL + blossom.vertices[blossom.vertices.index(connected_vert_l):blossom.vertices.index(connected_vert_r)+1] + [connected_vert_l] + stemR #O(n)
            else:
                blossom_path = stemL + [connected_vert_l] + blossom.vertices[blossom.vertices.index(connected_vert_r):blossom.vertices.index(connected_vert_l)+1] + stemR #O(n)
        else:
            # we need to take unmatched edge first
            if base not in blossom.matching_vertices:
                blossom_path = stemL + blossom.vertices[blossom.vertices.index(connected_vert_r):blossom.vertices.index(connected_vert_l)+1] + [connected_vert_l] + stemR
            else:
                blossom_path = stemL + [connected_vert_r] + blossom.vertices[blossom.vertices.index(connected_vert_l):blossom.vertices.index(connected_vert_r)+1] + stemR

    elif connected_vert_l is not None:
        if stemL[-1] in M.vertices:
            # we need to take unmatched edge first
            if base in blossom.matching_vertices:
                blossom_path = stemL + blossom.vertices[::-1]
            else:
                blossom_path = stemL + [connected_vert_l] + blossom.vertices[:-1]
        else:
            # we need to take the matched edge first
            if base in blossom.matching_vertices:
                blossom_path = stemL + [connected_vert_l] + blossom.vertices[:-1]
            else:
                blossom_path = stemL + blossom.vertices[::-1]
    elif connected_vert_r is not None:
        if stemR[0] in M.vertices:
            # we need to take unmatched edge first
            if base not in blossom.matching_vertices:
                blossom_path = blossom.vertices[1:] + [connected_vert_r] + stemR
            else:
                blossom_path = blossom.vertices[::-1] + stemR
        else:
            # we need to take the matched edge first
            if base not in blossom.matching_vertices:
                blossom_path = blossom.vertices[::-1] + stemR
    ...
    # then we actually lift the blossom
    G.remove_vertex(base) # O(n^2)
    M.remove_vertex(base) # O(n^2)
    for vertex in blossom.vertices: # we go past all edges twice and perform an operation costing O(n) time so we get O(m*n)
        G.add_vertex(vertex) # O(n)
        for edge in blossom.edges[vertex.name]: # O(e) amount of edges connected to v
            G.add_edge(vertex, edge) # O(n)
        M.add_vertex(vertex) # O(n)
        if vertex in blossom.matching: # O(n)
            for edge in blossom.matching[vertex.name]: # O(e) amount of edges connected to v
                M.add_edge(vertex, edge) # O(n)
                    ...
                    if w in forest.vertices: # O(n)
                        if distance(w, root(w))%2 == 1: # O(n)
                            ...
                            if root(v) != root(w): # O(n)
                                ...
                                path = findAugmentingPath(G, M) # recursion of max O(n) times
                                path = liftBlossom(G, M, blossom, path) # O(n^2)
                                return path
                    else:
