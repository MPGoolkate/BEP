...
class Graph:
    ...
    def add_edge(self, vertex1, vertex2):
        if vertex1 in self.vertices and vertex2 in self.vertices: # O(n)
            self.edges[vertex1].append(vertex2) # O(n)
            self.edges[vertex2].append(vertex1) # O(n)
    ...
    def remove_edge(self, vertex1, vertex2):
        if vertex1 in self.vertices and vertex2 in self.vertices: # O(n)
            if vertex2 in self.edges[vertex1] and vertex1 in self.edges[vertex2]: #O(n)
                self.edges[vertex1].remove(vertex2) #O(n^2)
                self.edges[vertex2].remove(vertex1) #O(n^2)
...
def liftBlossom(G, M, blossom):
    ...
    for neighbour in G.edges[base]: # max O(n) times so O(n^2) in total
        if neighbour in M.vertices: # O(n)
            stem = neighbour # O(1) 
            break
